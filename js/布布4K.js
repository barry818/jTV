...
// 内嵌线路排序函数（合并到当前文件，无外部依赖）
function sortPlayLine(sourceList) {
    const getPriority = (showName) => {
        const name = showName.toLowerCase();
        if (name.includes('4k')) return 1;
        if (name.includes('k')) return 2;
        if (name.includes('独家')) return 3;
        if (name.includes('秒播')) return 4;
        if (name.includes('自建')) return 5;
        if (name.includes('蓝光')) return 6;
        if (name.includes('专线')) return 7;
        return 8;
    };
    return [...sourceList].sort((a, b) => getPriority(a.show) - getPriority(b.show));
}
...
if (urls.length > 0) {
tempLines.push({ show: name, urls: urls.join('#') });
}
}
}

// 执行线路排序
tempLines = sortPlayLine(tempLines);

// 排序后还原为原格式
for(let item of tempLines){
    shows.push(item.show);
    play_urls.push(item.urls);
}
